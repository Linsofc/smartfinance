...
router.post('/', async (req, res) => {
  try {
    ...
    // Budget warning check for expense transactions
    let budgetWarnings = [];
    if (type === 'EXPENSE' && category) {
      ...
      if (budget) {
        ...
        const monthExpenses = await Transaction.aggregate([
          { $match: { userId: req.userId, category, type: 'EXPENSE', date: { $gte: startOfMonth, $lte: endOfMonth } } },
          { $group: { _id: null, total: { $sum: '$amount' } } }
        ]);
        const totalUsed = monthExpenses.length > 0 ? monthExpenses[0].total : 0;
        const percentage = (totalUsed / budget.amount) * 100;
        if (percentage >= 80) {
          budgetWarnings.push({ category, budget: budget.amount, used: totalUsed, percentage });
        }
      }
    }
    ...
    res.status(201).json({ message: 'Transaksi berhasil ditambahkan!', transaction, budgetWarnings });
  } catch (error) {
...
router.post('/import-csv', async (req, res) => {
  try {
    ...
    const fcontent = fs.readFileSync(csvPath, 'utf-8');
